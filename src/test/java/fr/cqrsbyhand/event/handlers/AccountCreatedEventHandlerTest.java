...
public class AccountCreatedEventHandlerTest {
  ...
  public void should_create_a_new_account_in_the_bank() throws Exception {
    ...
    doNothing().when(bank).createAccount(any(Account.class));

    accountCreatedEventHandler.handle(event);

    verify(bank).createAccount(new Account("abc", LocalDateTime.of(2017, Month.NOVEMBER, 23, 20, 0), LocalDateTime.of(2017, Month.NOVEMBER, 23, 20, 0), "My account", 0));
  }
}
